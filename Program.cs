...
class Program
{
    ...
    // Score starts at 0, plenty of room for improvement! !!BUG 1 START!!
    static int score = 0;

    // Time is of the essence in game development 
    //JuliaC., I changed the startingTime to a float to get the timer working :)
    static float startingTime = 1.00f;
    ...
    static void MovePlayer()
    {
        ...
        //JuliaC, changed -minus to %modulo width at the end of the line, to use modulo correctly 
        ...
        if (enemyMoveCounter + MOVE_FREQUENCY >= MOVE_FREQUENCY - 1)
        {
            foreach (var enemy in enemies)
            {
                ...
                if (enemy.X <= 0 || enemy.X >= width - 1)
                {
                    ...
                    //JuliaC, I rounded Y_MOVE_AMOUNT to (float)Math.Round(Y_MOVE_AMOUNT) :D
                    enemy.Y = (int)(enemy.Y + (float)Math.Round(Y_MOVE_AMOUNT));
                }
        ...
        for (int j = enemies.Count - 1; j >= 0; j--)
        {
            ...
            for (int i = bullets.Count - 1; i >= 0; i--)
            {
                if (bullets[i].X == enemies[j].X && bullets[i].Y == enemies[j].Y)
                {
                    ...
                    //JuliaC., I made the enemy defense divide by 2 (5/2 is 2int, times 3 shots is 6 player damage)
                    ...
                    if (enemies[j].Health <= 0)
                    {
                        ...
                        //JuliaC, byte was overflowing the score, changed data type in int
                        score += (int)random.Next(100, 150);
                    }
                    break;
                }
            }
...
class Player
{
    ...
    public int GetLevel()
    {
        ...
        //JuliaC., I changed Math.Round to Math.Cieling so the player always starts at lv.1
